...
scene.fog = new THREE.FogExp2(0x010310, 0.00045);
...
function createFlowerTexture(type) {
    ...
    gradient.addColorStop(0, '#9DF5FF');
    gradient.addColorStop(0.5, '#4F9DFF');
    gradient.addColorStop(1, '#0047FF');
    ...
    centerGrad.addColorStop(0, '#001133');
    centerGrad.addColorStop(0.6, '#000B22');
    centerGrad.addColorStop(1, '#00030A');
...
for (let i = 0; i < TOTAL_FLOWERS; i++) {
    ...
    let r = 0.1 + Math.random() * 0.2, g = 0.4 + Math.random() * 0.4, b = 0.8 + Math.random() * 0.2;
    if (distFactor < 0.3) { r = 0.0; g = 0.2 + Math.random() * 0.2; b = 1.0; }
    if (distFactor > 0.7) { r = 0.4; g = 0.8; b = 1.0; }
    colors[i * 3] = r;
    colors[i * 3 + 1] = g;
    colors[i * 3 + 2] = b;
...
const fragmentShader = `
    ...
    void main() {
        ...
        finalColor += vec3(0.04, 0.12, 0.2); // Tinte azul
...
function createSunflower(x, z, yOffset = 0, scaleFactor = 1.0) {
    ...
    const stemMat = new THREE.MeshStandardMaterial({ color: 0x224c18, roughness: 0.8, metalness: 0.1 });
    const stem = new THREE.Mesh(stemGeo, stemMat);
    group.add(stem);

    const leafMat = new THREE.MeshStandardMaterial({ color: 0x336223, roughness: 0.9, side: THREE.DoubleSide });
    ...
    const baseMat = new THREE.MeshStandardMaterial({ color: 0x224c18, roughness: 0.9 });
    ...
    const petalMat = new THREE.MeshStandardMaterial({
        color: 0x0033cc,
        emissive: 0x0088ff,
        emissiveIntensity: 0.25, // Strong glow
        roughness: 0.4,
...
const pollenMat = new THREE.PointsMaterial({ color: 0xbbeeff, size: 0.04, transparent: true, blending: THREE.AdditiveBlending, opacity: 0.8 });
...
const dustMatObj = new THREE.PointsMaterial({ color: 0x77bbff, size: 0.055, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending });
...
const coreGlow = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0x33aaff, emissive: 0x0055cc, emissiveIntensity: 0.8, roughness: 0.5 })
);
scene.add(coreGlow);
...
const haloMatPoints = new THREE.PointsMaterial({ color: 0x6699ff, size: 0.06, blending: THREE.AdditiveBlending, opacity: 0.6 });
const haloRing = new THREE.Points(haloGeo, haloMatPoints);
scene.add(haloRing);

// Luces azules
const ambientLight = new THREE.AmbientLight(0x001122, 1.2);
scene.add(ambientLight);
const pointLight = new THREE.PointLight(0x99ddff, 1.0, 30);
pointLight.position.set(2, 2.5, 2);
scene.add(pointLight);
const backLight = new THREE.PointLight(0x66aaff, 0.5);
backLight.position.set(-3, 2, -5);
scene.add(backLight);
const fillLight = new THREE.PointLight(0xbbeeff, 0.4);
...
console.log('tu galaxia de flores azules siiii :))))');
