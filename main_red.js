...
scene.background = new THREE.Color(0x120101);
scene.fog = new THREE.FogExp2(0x120101, 0.00045);
...
function createFlowerTexture(type) {
    ...
    gradient.addColorStop(0, '#FF9DAA');
    gradient.addColorStop(0.5, '#FF4F6A');
    ...
    centerGrad.addColorStop(0, '#330005');
    centerGrad.addColorStop(0.6, '#220003');
    centerGrad.addColorStop(1, '#0A0000');
...
for (let i = 0; i < TOTAL_FLOWERS; i++) {
    ...
    let r = 0.8 + Math.random() * 0.2, g = 0.1 + Math.random() * 0.2, b = 0.2 + Math.random() * 0.2;
    if (distFactor < 0.3) { r = 1.0; g = 0.0; b = 0.1 + Math.random() * 0.1; }
    if (distFactor > 0.7) { r = 1.0; g = 0.4; b = 0.5; }
    colors[i * 3] = r;
    colors[i * 3 + 1] = g;
    colors[i * 3 + 2] = b;
...
const fragmentShader = `
    ...
    void main() {
        ...
        finalColor += vec3(0.2, 0.04, 0.08); // Tinte rojo
...
const pollenMat = new THREE.PointsMaterial({ color: 0xffbbcc, size: 0.04, transparent: true, blending: THREE.AdditiveBlending, opacity: 0.8 });
...
const dustMatObj = new THREE.PointsMaterial({ color: 0xff7799, size: 0.055, transparent: true, opacity: 0.3, blending: THREE.AdditiveBlending });
...
const coreGlow = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xff4455, emissive: 0xcc0022, emissiveIntensity: 0.8, roughness: 0.5 })
);
scene.add(coreGlow);
...
const haloMatPoints = new THREE.PointsMaterial({ color: 0xff6688, size: 0.06, blending: THREE.AdditiveBlending, opacity: 0.6 });
const haloRing = new THREE.Points(haloGeo, haloMatPoints);
scene.add(haloRing);

// Luces rojas
const ambientLight = new THREE.AmbientLight(0x220005, 1.2);
scene.add(ambientLight);
const pointLight = new THREE.PointLight(0xff99aa, 1.0, 30);
pointLight.position.set(2, 2.5, 2);
scene.add(pointLight);
const backLight = new THREE.PointLight(0xff6677, 0.5);
backLight.position.set(-3, 2, -5);
scene.add(backLight);
const fillLight = new THREE.PointLight(0xffeebb, 0.4);
